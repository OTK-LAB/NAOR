    ...
    */
    ...
    void Update()
    {
        ...
        if (platforms == Platforms.MovingPlatformY && !loop)
        {
            ...
            Sequence mySequence = DOTween.Sequence();
            mySequence.Append(TargetObj.transform.DOLocalMoveY(firstDestination, firstDuration).SetEase(Ease.InOutSine));//5,2
            mySequence.Append(TargetObj.transform.DOLocalMoveY(secondDestination, secondDuration).SetEase(Ease.InOutSine));//0,2
            mySequence.SetLoops(-1, LoopType.Restart);
        }
