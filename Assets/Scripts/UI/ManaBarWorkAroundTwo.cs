...
using DG.Tweening;
using UltimateCC;
public class ManaBarWorkAroundTwo : MonoBehaviour
{
	...
	{
		if(value - slider.maxValue >= 0)
		{
			DOTween.To(() => slider.value, x => slider.value = x, value - slider.maxValue, easingDuration);
		}
		else
		{
			slider.value = 0;
